...
echo cd /workspace_build/build_linux_gcc_shared ^&^& ctest -T memcheck --output-on-failure
...
echo export CC=gcc
echo export CXX=g++
echo cmake -S /workspace_src -B /workspace_build/build_linux_gcc_static -DCMAKE_BUILD_TYPE="${BUILD_TYPE}" -DBUILD_SHARED_LIBS=OFF -DCMAKE_INTERPROCEDURAL_OPTIMIZATION=ON -DCDD_CHARSET=ANSI -DCDD_THREADING=OFF -DCDD_DEPS=FETCHCONTENT -DBUILD_TESTING=ON -DCDD_MSVC_RTC=OFF %*
...
echo cd /workspace_build/build_linux_gcc_static ^&^& ctest -T memcheck --output-on-failure
echo cd /workspace_build
echo.
echo echo "======================================================================"
...
echo cmake -S /workspace_src -B /workspace_build/build_linux_clang_static -DCMAKE_BUILD_TYPE="${BUILD_TYPE}" -DBUILD_SHARED_LIBS=OFF -DCMAKE_INTERPROCEDURAL_OPTIMIZATION=ON -DCDD_CHARSET=ANSI -DCDD_THREADING=OFF -DCDD_DEPS=SYSTEM -DBUILD_TESTING=ON -DCDD_MSVC_RTC=OFF %*
...
echo cd /workspace_build/build_linux_clang_static ^&^& ctest -T memcheck --output-on-failure
echo cd /workspace_build
echo.
echo echo "======================================================================"
...
echo cd /workspace_build/build_linux_clang_shared ^&^& ctest -T memcheck --output-on-failure
echo cd /workspace_build
) > "!RUN_SCRIPT!"

echo Running container !CONTAINER_NAME!...
docker run --name !CONTAINER_NAME! -v "%CD%:/workspace_build" -v "!SRC_DIR!:/workspace_src" -v "C:/Users/samue/repos/vcpkg:/vcpkg" !IMAGE_NAME! bash -c "tr -d '\r' < /workspace_src/.run_ubuntu_tests.sh | bash"
